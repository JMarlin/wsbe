...
void Window_apply_bound_clipping(Window* window) {
    ...
    Window_apply_bound_clipping(window->parent);
...
void Window_paint(Window* window) {
    ...
    Window_apply_bound_clipping(window);
    ...
    for(i = 0; i < window->children->count; i++) {
        ...
        temp_rect = Rect_new(current_child->y, current_child->x,
                             current_child->y + current_child->height - 1,
                             current_child->x + current_child->width - 1);
        Context_subtract_clip_rect(window->context, temp_rect);
        free(temp_rect);
    }
